...
class App extends Component {
  ...
  onCloseBooking = () => this.setState(() => ({ selectedBooking: null }));
  ...
  onToggleRecurring = value => {
    ...
    if (value === "none") disableRecurring = true;
    else disableRecurring = false;
    this.setState({ disableRecurring: disableRecurring });
  };

  onSetFloorParam = value => this.setState({ floorParam: value });
  ...
  oneSetCurrentDateBookings = () => {
    ...
    roomData.forEach(room => {
      ...
      room.bookings.forEach(booking => {
        ...
        if (bookingStart === currentDate) todaysBookings.push(booking);
      });
    });
